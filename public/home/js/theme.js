...
$('.collect').click(function () {
	var _this = $(this);
	var article_id = Number(_this.attr('article_id'));
	if (_this.attr('uid') && _this.hasClass('collect-no')) {
		...
		$.ajax({
			...
			dataType: 'html',
			url: 'collect',
			data: 'uid=' + uid + '&article_id=' + article_id + '&act=add',
			cache: false,
			success: function () {
		...
		$.ajax({
			...
			dataType: 'html',
			url: 'collect',
			data: 'uid=' + uid + '&article_id=' + article_id + '&act=remove',
			cache: false,
			success: function () {
...
$('.buysaledl').click(function () {
	...
	var login_in = 0;
	$.ajax({
		type: 'POST', dataType: 'json', url: tin.ajax_url, data: 'action=checklogin', cache: false, success: function (check) {
			login_in = check.status;
			if (login_in != 0) {
				$.ajax({
					type: 'POST', dataType: 'json', url: tin.ajax_url, data: 'action=popsaledl&sid=' + sid + '&pid=' + pid + '&uid=' + uid, cache: false, success: function (msg) {
...
function Confirm_to_buy() {
	...
	uid = $('.downldlinks-inner').attr('uid');
	$.ajax({
		type: 'POST', dataType: 'json', url: tin.ajax_url, data: 'action=confirmbuy&sid=' + sid + '&pid=' + pid + '&uid=' + uid, cache: false, success: function (msg) {
...
$('button#subscribe').click(function () {
	...
	if (email == '' || (!email.match('^([a-zA-Z0-9_-])+((\.)?([a-zA-Z0-9_-])+)+@([a-zA-Z0-9_-])+(\.([a-zA-Z0-9_-])+)+$'))) {
		...
		$.ajax({
			type: 'POST', dataType: 'json', url: tin.ajax_url, data: 'action=subscribe&email=' + email, cache: false, success: function () {
...
$('button#unsubscribe').click(function () {
	...
	if (email == '' || (!email.match('^([a-zA-Z0-9_-])+((\.)?([a-zA-Z0-9_-])+)+@([a-zA-Z0-9_-])+(\.([a-zA-Z0-9_-])+)+$'))) {
		...
		$.ajax({
			type: 'POST', dataType: 'json', url: tin.ajax_url, data: 'action=unsubscribe&email=' + email, cache: false, success: function (data) {
	...
	function ajax_load_data(e, p) {
		...
		$.ajax({
			...
			success: function (out) {
				...
				setTimeout(function () {
					$loader.fadeOut(); result.imagesLoaded(function () {
						result.fadeIn(2000);
